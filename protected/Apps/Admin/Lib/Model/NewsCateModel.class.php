...
class NewsCateModel extends Model {
    ...
    {
        $m = M($tbname);
        $condition['path'] = array('like', '%' . $cate_path . '%');
...
//        exit;
        foreach ($result as $k => $v) {
            if ($cate_path == ',')
                unset($cate_path);
            $data['path'] = substr($v['path'], strpos($v['path'], ','.$cate_id . ','), strlen($v['path']));
            $json = array('status' => '1', 'info' => $cate_path .'++++'.$cate_id.'===='.$data['path']);
        echo json_encode($json);
       exit;
//$m->where('cat_id=' . intval($v['cat_id']))->save($data);
        }
